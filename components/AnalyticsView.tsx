  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="glass-panel p-5 rounded-2xl shadow-lg border-white/5 relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-8 bg-emerald-500/10 rounded-full blur-xl group-hover:bg-emerald-500/20 transition-colors"></div>
          ...
          <p className="text-3xl font-bold text-white relative z-10 neon-text-emerald">KES {data.projected_savings}</p>
          <p className="text-xs text-slate-400 relative z-10 mt-1">Projected this month</p>
        </div>

        <div className="glass-panel p-5 rounded-2xl shadow-lg border-white/5 relative overflow-hidden group">
           <div className="absolute top-0 right-0 p-8 bg-indigo-500/10 rounded-full blur-xl group-hover:bg-indigo-500/20 transition-colors"></div>
      ...
      <div className="glass-panel p-6 rounded-3xl shadow-xl border-white/5">
          ...
          {data.weekly_spending_trend.map((item, i) => (
            <div key={i} className="flex flex-col items-center flex-1 group">
              <div className="relative w-full h-full flex items-end">
                <div 
                  className="w-full bg-slate-800 rounded-t-lg relative overflow-hidden transition-all duration-500 ease-out group-hover:shadow-[0_0_10px_rgba(16,185,129,0.3)]"
                  style={{ height: `${(item.amount / maxSpend) * 100}%`, minHeight: '10%' }}
                >
                   <div className="absolute inset-0 bg-gradient-to-t from-emerald-600 to-emerald-400 opacity-80 group-hover:opacity-100 transition-opacity"></div>
                </div>
              </div>
              <span className="text-[10px] text-slate-500 mt-3 font-bold uppercase tracking-wider group-hover:text-emerald-400 transition-colors">{item.day.slice(0,3)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Market Alerts */}
      <div className="glass-panel p-5 rounded-2xl border-l-4 border-l-orange-500 bg-orange-500/5">
        ...
        <ul className="space-y-3">
          {data.price_alerts.map((alert, i) => (
            <li key={i} className="flex items-start gap-3 text-sm text-slate-300">
              <span className="mt-1.5 w-1.5 h-1.5 bg-orange-500 rounded-full shadow-[0_0_5px_orange]"></span>
              {alert}
            </li>
          ))}
        </ul>
